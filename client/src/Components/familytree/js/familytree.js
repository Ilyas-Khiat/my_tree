...
class FamilyTree extends FTDrawer {
    ...
    draw_data(data) {
        ...
        d3.selectAll('.node') // Make sure '.node' matches your actual node class or identifier
        .on("contextmenu", (event, d) => {
            ...
            d3.select('#add-relation').on('click', () => addRelation(d));
            d3.select('#add-parent').on('click', () => addParent(d));

            // Prevent the context menu from staying open
            d3.select('body').on('click', () => {
                d3.select('#context-menu').style('display', 'none');
